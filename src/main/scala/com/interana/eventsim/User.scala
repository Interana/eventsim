...
class User(val alpha: Double, // alpha = expected session length
           val beta: Double,  // beta = expected number of pages per session
           val gamma:Double,  // gamma = expected session inter-arrival time
  ...
  var session = new Session(Session.pickFirstTimeStamp(startTime, alpha, gamma), alpha, beta, gamma, initialState)
